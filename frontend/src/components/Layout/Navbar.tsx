      ...
      <Toolbar sx={{ maxWidth: 1280, width: '100%', mx: 'auto', gap: 2, px: { xs: 2, sm: 3 } }}>
        ...
        {isAuthenticated && (
          <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', gap: 0.5, flex: 1, justifyContent: 'center' }}>
            {[
              { to: '/movies',      icon: <Film size={15} />,           label: 'Movies'      },
              { to: '/my-bookings', icon: <Ticket size={15} />,         label: 'My Tickets'  },
              ...(isAdmin ? [{ to: '/admin', icon: <LayoutDashboard size={15} />, label: 'Admin' }] : []),
            ].map(({ to, icon, label }) => (
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
          ...
          {isAuthenticated ? (
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <MuiButton component={Link} to="/login" variant="outlined" size="small">Login</MuiButton>
              <MuiButton component={Link} to="/register" variant="contained" size="small" sx={{ display: { xs: 'none', sm: 'flex' } }}>Sign Up</MuiButton>
            </Box>
          )}
        </Box>
      </Toolbar>
